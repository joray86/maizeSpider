...
def formatData(Item):
    title = Item["title"][0].replace(u'\xa0', u'')
    ...
    for line in priceDesc:
        ...
        pattern_special1 = re.compile(
            r'' + companyList3[0] + '.*?(\d{4})-(\d{4})元/吨' + '.*?' + companyList3[1] + '.*?(\d*)元/吨')
        pattern_special2 = re.compile(
            r'' + companyList3[0] + '.*?(\d*)元/吨' + '.*?' + companyList3[1] + '.*?(\d{4})-(\d{4})元/吨')
        pattern_special3 = re.compile(
            r'' + companyList3[0] + '.*?(\d{4})-(\d{4})元/吨' + '.*?' + companyList3[1] + '.*?(\d*)元/吨')
        ...
        if (result):
            ...
            priceDict['青岛地区' + companyList3[1]] = (float(result_special2.group(2)) + float(
                result_special2.group(3))) / 4000
        elif (result_special3):
            priceDict[companyList3[0]] = (float(result_special3.group(1)) + float(result_special3.group(2))) / 4000
            priceDict['青岛地区' + companyList3[1]] = (float(result_special3.group(3)) + float(
                result_special3.group(4))) / 4000
